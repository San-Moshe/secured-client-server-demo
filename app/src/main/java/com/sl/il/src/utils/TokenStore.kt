...
class TokenStore @Inject constructor(private val sharedPref: SharedPreferences) {
    ...
    private fun encryptToken(token: String): ByteArray =
        Cipher.getInstance("AES/GCM/NoPadding").apply {
            ...
            sharedPref.edit().putString(INITIALIZATION_VECTOR, Base64.encodeToString(iv, Base64.DEFAULT)).apply()
            doFinal(token.encodeToByteArray())
        }

    private fun decryptToken(encryptedToken: ByteArray?): String =
        Cipher.getInstance("AES/GCM/NoPadding").apply {
            init(Cipher.DECRYPT_MODE, secretKey, GCMParameterSpec(128, Base64.decode(sharedPref.getString(
                INITIALIZATION_VECTOR, ""), Base64.DEFAULT)))
        }.run {
    ...
    fun storeToken(token: String) {
        encryptToken(token).let { encrypted ->
            sharedPref.edit().putString(TOKEN_PREF_KEY, Base64.encodeToString(encrypted, Base64.DEFAULT)).commit()
        }
    }
